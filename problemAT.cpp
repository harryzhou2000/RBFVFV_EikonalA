...
namespace ScalarCfv
{
	bool problemAT::timeMarchingExplicitSSPRK(
		vertexVector* node,
	...
	bool problemAT::getGRE(GaussIntegralCellO1Grid<vO>* gaussIntegralCell, real & L1, real & L2, real & L8){
		real L1t = 0.0;
		real L2t = 0.0;
		real L8t = 0.0;
		real volt = 0.0;
		...
		for (iterCell = cellFieldData_->begin(); iterCell != cellFieldData_->end(); ++iterCell){
			if ((*iterCell).baryCenter.x < 10.0 || (*iterCell).baryCenter.x > 50.0
				|| (*iterCell).baryCenter.y < 1.0 || (*iterCell).baryCenter.y > 19.0){
				continue;
			}
