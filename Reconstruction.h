...
namespace ScalarCfv
{
	...
	public:
		...
		virtual bool initBaseMomentAndRelaxFactor(
			parameter* parameter,
		...
		virtual bool initFaceWeight(
			parameter *parameter,
			cellFieldDataVector *cellFieldData,
			faceFieldDataVector *faceFieldData);

		virtual bool initReconstructionMatrixAndVector(
			parameter *parameter,
			cellFieldDataVector *cellFieldData,
			cellGaussDataVector *cellGaussData,
			faceFieldDataVector *faceFieldData,
		...
		virtual bool getBoundaryValueCheck(
			parameter *parameter,
			cellFieldDataVector *cellFieldData,
			cellGaussDataVector *cellGaussData,
			faceFieldDataVector *faceFieldData,
		...
		virtual bool excuteReconstruction(
			parameter *parameter,
			cellFieldDataVector *cellFieldData,
			cellGaussDataVector *cellGaussData,
			faceFieldDataVector *faceFieldData,
		...
		//CR
		virtual bool initReconstructionMatrixAndVectorCR(
			parameter *parameter,
			cellFieldDataVector *cellFieldData,
			cellGaussDataVector *cellGaussData,
			faceFieldDataVector *faceFieldData,
		...
		virtual bool excuteReconstructionCR(
			parameter *parameter,
			cellFieldDataVector *cellFieldData,
			cellGaussDataVector *cellGaussData,
			faceFieldDataVector *faceFieldData,
...
//----------------------------------------------------------------------------------
	template<unsigned int O>
	bool reconstruction<O>::initBaseMomentAndRelaxFactor(
		parameter* parameter,
		cellFieldDataVector* cellFieldData,
		cellGaussDataVector* cellGaussData,
		GaussIntegralCellO1Grid<vO>* gaussIntegralCell){
		...
		for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
			...
			//base momment
			tensor1D<real, NDOFS>* phi
				= new tensor1D<real, NDOFS>[static_cast<int>((*iterCellFieldData).PG)];
			//------------------------------------------
			...
			for (int ii = 0; ii < static_cast<int>((*iterCellFieldData).PG); ++ii){
				...
				CfvMath::getMoment(
					(*iterCellFieldData).gaussPairVector_[ii].p,
					(*iterCellFieldData).baryCenter,
					(*iterCellFieldData).lengthReference,
					phiG);
				for (int jj = 1; jj < NDOFS; ++jj){//ע�������ָ�귶Χ����20200315
...
//----------------------------------------------------------------------------------
	...
	template<unsigned int O>
	bool reconstruction<O>::initFaceWeight(
		parameter* parameter,
		cellFieldDataVector* cellFieldData,
		faceFieldDataVector* faceFieldData){
		...
		for (iterFaceFieldData = faceFieldData->begin(); iterFaceFieldData != faceFieldData->end(); ++iterFaceFieldData){
			...
			point midPoint = 0.5 * ((*iterFaceFieldData).faceNode[1].second
				+ (*iterFaceFieldData).faceNode[2].second);
			...
			for (int ii = 0; ii < static_cast<int>((*iterFaceFieldData).NDOFS); ++ii){//0: for b_vec
				...
				(*iterFaceFieldData).faceWeightVF[ii] =
					w[ii] * (*iterFaceFieldData).weightVF
					* std::pow(dx, 2 * m) * std::pow(dy, 2 * n)
					* std::pow(Cm, 2) / std::pow(Pmn, 2);
...
//				//	int n = CfvMath::nMapping[ii];
//				//	real Cm = CfvMath::getCombination(m, n);
//				//	//real Cn = CfvMath::getCombination(n, m);
//				//	real Pmn = CfvMath::getFactorial(m, n);
//				//
//				//	(*iterFaceFieldData).faceWeightVF[ii] =
//				//		w[ii] * (*iterFaceFieldData).weightVF //(*iterFaceFieldData).weightVF / (*iterFaceFieldData).area
//				//		* std::pow(delta.x, 2 * m) * std::pow(delta.y, 2 * n)
//				//		* std::pow(Cm, 2) / std::pow(Pmn, 2);//* std::pow(Cn, 2)
//				//	std::cout << "-----------------" << std::endl;
//				//	std::cout << w[ii] << "\t" << (*iterFaceFieldData).weightVF << std::endl;
//				//	std::cout << (*iterFaceFieldData).index << std::endl;
//				//	std::cout << m << "\t" << n << std::endl;
//				//	std::cout << std::pow(delta.x, 2 * m) <<  "\t" << std::pow(delta.y, 2 * n) << std::endl;
//				//	std::cout << std::pow(Cm, 2) << "\t" << std::pow(Pmn, 2) << std::endl;
//				//	std::cout << (*iterFaceFieldData).faceWeightVF[ii] << std::endl;
//				//}
//
//				//iterCellFieldData = cellFieldData->begin() + cl - 1;
//				//point scaleL = (*iterCellFieldData).lengthReference;
//				//iterCellFieldData = cellFieldData->begin() + cr - 1;
//				//point scaleR = (*iterCellFieldData).lengthReference;
//				//delta.x = CfvMath::harmonic(scaleL.x, scaleR.x);
//				//delta.y = CfvMath::harmonic(scaleL.y, scaleR.y);
//
//			}
//			else if (cr == Wall){
//				if (parameter->isViscous == true){
...
//				//correct			
//
//				refLR = (omegaL + omegaR) / (*iterFaceFieldData).area / 2.0;
//
//				for (int ii = 0; ii < static_cast<int>((*iterFaceFieldData).NDOFS); ++ii){
//					w[ii] = 1.0;
//				}
...
//				//1 tbl
//				//(*iterFaceFieldData).faceWeightVF[ii] =
//				//	w[ii] * (*iterFaceFieldData).weightVF * (*iterFaceFieldData).area /// refLR ///// refLR // / (*iterFaceFieldData).area //// / refLR // 
//				//	* std::pow(refLR, 2 * (m + n)) * std::pow(Cm, 2) / std::pow(Pmn, 4);
//
//				//2 bfs
//				//(*iterFaceFieldData).faceWeightVF[ii] =
//				//	w[ii] * (*iterFaceFieldData).weightVF * (*iterFaceFieldData).area /// refLR ///// refLR // / (*iterFaceFieldData).area //// / refLR // 
...
//				//3 0012 sub
//				//(*iterFaceFieldData).faceWeightVF[ii] =
//				//	w[ii] * (*iterFaceFieldData).weightVF * (*iterFaceFieldData).area //// / refLR // 
//				//	* std::pow(refLR, 2 * (m + n)) * std::pow(Cm, 2) / std::pow(Pmn, 4);
//
//				//4 0012 trans
//				(*iterFaceFieldData).faceWeightVF[ii] =
//					w[ii] * (*iterFaceFieldData).weightVF * (*iterFaceFieldData).area * (*iterFaceFieldData).area //// / refLR // 
//					* std::pow(refLR, 2 * (m + n));
//
//				//5 0012 cd blow up
//				//(*iterFaceFieldData).faceWeightVF[ii] =
...
//				//6 30p30n
//				//(*iterFaceFieldData).faceWeightVF[ii] =
//				//	w[ii] * (*iterFaceFieldData).weightVF * (*iterFaceFieldData).area / Omega //// / refLR // 
//				//	* std::pow(refLR, 2 * (m + n));
//
//				//super circle
//				//(*iterFaceFieldData).faceWeightVF[ii] =
...
//----------------------------------------------------------------------------------
	template<unsigned int O>
	bool reconstruction<O>::initReconstructionMatrixAndVector(
		parameter* parameter,
		cellFieldDataVector* cellFieldData,
		cellGaussDataVector* cellGaussData,
		faceFieldDataVector* faceFieldData,
		...
		for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
			...
			for (int ff = 1; ff < (*iterCellFieldData).cellFaceNumber + 1; ++ff){//ע��ѭ�����ޣ���20200314
				...
				if (cr > 0){
					...
					tensor2D<real, NDOFS, NDOFS>* fII
						= new tensor2D<real, NDOFS, NDOFS>[static_cast<int>((*iterFaceFieldData_).fPG)];
					tensor2D<real, NDOFS, NDOFS>* fIJ
						= new tensor2D<real, NDOFS, NDOFS>[static_cast<int>((*iterFaceFieldData_).fPG)];
					tensor1D<real, NDOFS>* fI
						= new tensor1D<real, NDOFS>[static_cast<int>((*iterFaceFieldData_).fPG)];
					...
					for (int gg = 0; gg < static_cast<int>((*iterFaceFieldData_).fPG); ++gg){
						...
						for (int ll = 1; ll < static_cast<int>(NDOFS); ++ll){
							for (int rr = 1; rr < static_cast<int>(NDOFS); ++rr){
								...
								for (int kk = 0; kk < static_cast<int>(NDOFS); ++kk){
									fII[gg][ll][rr] += (*iterFaceFieldData_).faceWeightVF[kk]
										* matrixDiffBaseI[ll][kk] * matrixDiffBaseI[rr][kk];
									fIJ[gg][ll][rr] += (*iterFaceFieldData_).faceWeightVF[kk]
										* matrixDiffBaseI[ll][kk] * matrixDiffBaseJ[rr][kk];
								}
							}
						}
						//vector
			...
			for (int cc = 1; cc < (*iterCellFieldData).cellCellNumber + 1; ++cc){//cellFaceNumber -> cellCellNumber
	...
	template <unsigned int O>
	bool reconstruction<O>::getBoundaryValueCheck(
		parameter *parameter,
		cellFieldDataVector *cellFieldData,
		cellGaussDataVector *cellGaussData,
		faceFieldDataVector *faceFieldData,
...
//----------------------------------------------------------------------------------
	template<unsigned int O>
	bool reconstruction<O>::excuteReconstruction(
		parameter* parameter,
		cellFieldDataVector* cellFieldData,
		cellGaussDataVector* cellGaussData,
		faceFieldDataVector* faceFieldData,
	...
	//----------------------------------------------------------------------------------
	template<unsigned int O>
	bool reconstruction<O>::initReconstructionMatrixAndVectorCR(
		parameter* parameter,
		cellFieldDataVector* cellFieldData,
		cellGaussDataVector* cellGaussData,
		faceFieldDataVector* faceFieldData,
			...
			for (int ff = 1; ff < (*iterCellFieldData).cellFaceNumber + 1; ++ff){//ע��ѭ�����ޣ���20200314
				...
				if (cr > 0){
					...
					for (int gg = 0; gg < static_cast<int>((*iterFaceFieldData_).fPG); ++gg){
						...
						for (int ll = 1; ll < static_cast<int>(NDOFS); ++ll){
							for (int rr = 1; rr < static_cast<int>(NDOFS); ++rr){
								fII[gg][ll][rr] = (*iterFaceFieldData_).faceWeightVF[0]
									* vectorBaseI[ll] * vectorBaseI[rr];
							}
						}
					}
					//get the face Gauss point for coefficient matrix. integral along the interface between cell i and cell j
...
//		std::cout << " ..CR reconstruction matrix and vector has been initialized." << std::endl;
	...
	template<unsigned int O>
	bool reconstruction<O>::excuteReconstructionCR(
		parameter* parameter,
		cellFieldDataVector* cellFieldData,
		cellGaussDataVector* cellGaussData,
		faceFieldDataVector* faceFieldData,
					...
					for (int ll = 1; ll < static_cast<int>(NDOFS); ++ll){
						...
						for (int gg = 0; gg < static_cast<int>((*iterFaceFieldData_).fPG); ++gg){
							f[gg] = (0.5*(1.0 + signLR)*fIL[gg] + 0.5*(1.0 - signLR)*fIR[gg] - valueL)
								* fIBase[gg][ll] * (*iterFaceFieldData_).faceWeightVF[0];
