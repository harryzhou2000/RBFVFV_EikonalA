...
namespace ScalarCfv
{
	...
	bool grid::readGridFile(const std::string & filename){
		...
		std::cout << "------------------------------------------------------------------" << std::endl;
		std::cout << "	cellNumber:" << "\t" << parameter_->cellNumber << std::endl;
		std::cout << "	nodeNumber:" << "\t" << parameter_->nodeNumber << std::endl;
		...
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			...
			if ((*iterCell).cellFaceNumber == 3){				//cellNodeNumber
				(*iterCell).cellType_ = Triangle;				//cellType_
				(*iterCell).cellNodeNumber = 3;					//cellNode.first
				(*iterCell).cellFaceIndex.resize(3+1);			//分配cellFaceIndex
				(*iterCell).cellNode.resize(3+1);					//分配cellNode
			}
			...
			for (int jj = 1; jj < (*iterCell).cellNodeNumber + 1; ++jj){
				...
				//(*iterCell).cellNode[jj].first -= 1;			//检查是否正确！！20200220
			}
			++ii;
		}
		...
		ii = 0;
		for (iterNode = node_->begin(); iterNode != node_->end(); ++iterNode){					//index
		...
		for (iterBoundary = boundary_->begin(); iterBoundary != boundary_->end(); ++iterBoundary){
			...
			if (boundaryTypeIn == 1){							//boundaryTag
		...
		if (parameter_->isPeriodicBoundary){
			...
			for (ii = 1; ii < boundaryNumberPeriodic+1; ++ii){
				fileIn >> boundaryPairPeriodicTag[ii].first; //tag1 从1开始
				...
				for (iterBoundary = boundary_->begin(); iterBoundary != boundary_->end(); ++iterBoundary){
					if ((*iterBoundary).boundaryTag == boundaryPairPeriodicTag[ii].first
						|| (*iterBoundary).boundaryTag == boundaryPairPeriodicTag[ii].second){
	...
	bool grid::allocateCell(){
		...
		cellVector::iterator iterCell;
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			(*iterCell).cellCellNumber = (*iterCell).cellFaceNumber;//此处暂时这样初始化 20200219
			(*iterCell).cellCellIndex.resize((*iterCell).cellCellNumber + 1);//注意此处！(*iterCell).cellCellNumber->current cell
		...
		//cellCellNumber
		//分配cellCellIndex
		//分配&初始化cellFaceSideOff
		//分配&初始化cellFaceWeight
		return true;
	}
	...
	bool grid::setNodeTopology(){
		...
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			for (int jj = 1; jj < (*iterCell).cellFaceNumber+1; ++jj){
				iterNode = node_->begin();//通过迭代器找到元素
				
				iterNode += (*iterCell).cellNode[jj].first - 1;//注意指针移动是否正确！！20200220
		...
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			for (int jj = 1; jj < (*iterCell).cellFaceNumber+1; ++jj){
				iterNode = node_->begin();//通过迭代器找到元素
				iterNode += (*iterCell).cellNode[jj].first - 1;//注意指针移动是否正确！！20200220
				(*iterNode).nodeCellNumber += 1;//注意自加位置！！20200225
		...
		for (iterNode = node_->begin(); iterNode != node_->end(); ++iterNode){
			...
			//}
			for (int jj = 0; jj < 2 * (*iterNode).nodeCellNumber + 1; ++jj){//注意确认这里，为啥2倍
	...
	int grid::checkFace(int n1, int n2){
		vertexVector::iterator iterNode;
		faceVector::iterator iterFace;
		iterNode = node_->begin() + n1 - 1;//注意指针移动是否正确！！20200220//20200225确认，不用-1，因为从0开始
		for (int ii = 1; ii < (*iterNode).nodeCellfaceNumber + 1; ++ii){
			int f1 = (*iterNode).nodeCellfaceIndex[ii];
			iterFace = face_->begin() + f1 - 1;//注意指针移动是否正确！！20200220
			bool isTrue1 = ((*iterFace).faceNode[1].first == n1)
				&& ((*iterFace).faceNode[2].first == n2);
			bool isTrue2 = ((*iterFace).faceNode[1].first == n2)
				&& ((*iterFace).faceNode[2].first == n1);
			if (isTrue1 || isTrue2){
				return f1;
			}
		}
	...
	bool grid::setFaceTopology(){
		...
		faceVector::iterator iterFace;
		for (iterFace = face_->begin(); iterFace != face_->end(); ++iterFace){					//index
		...
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			for (int jj = 1; jj < (*iterCell).cellFaceNumber+1; ++jj){
				int n1 = (*iterCell).cellNode[jj].first; //是编号！
				...
				if (faceIndex == 0){
					...
					if ((*iterNode).nodeCellfaceNumber > 50){//nodeFaceNumber 此处的为局部的，是为了建立face关系用的，因为按单元循环！！
						std::cout << "	error: node is shared by too many edges/faces!" << std::endl;
						exit(1);
					}
					(*iterNode).nodeCellfaceNumber += 1;//注意自加位置！！20200225
			...
			if ((*iterCell).index == parameter_->cellNumber){// [编号+1=数目]//20200225注意这里的编号是否有问题
				faceNumberCheck = ii;//编号！
			}
		}
		...
		for (iterFace = face_->begin(); iterFace != face_->end(); ++iterFace){
			...
			for (int jj = 1; jj < 3; ++jj){
				...
				iterNode = node_->begin() + n1 - 1;//通过迭代器找到元素
				//iterNode += (n1 - 1);
				(*iterNode).nodeFaceNumber += 1;
	...
	bool grid::setBoundaryTopology(){
		...
		for (iterBoundary = boundary_->begin(); iterBoundary != boundary_->end(); ++iterBoundary){
			...
			iterNode += (n1 - 1);
			for (jj = 1; jj < (*iterNode).nodeFaceNumber+1; ++jj){//有问题！！20200228
		...
		if (parameter_->isPeriodicBoundary){
			...
			for (int ii = 1; ii < boundaryNumberPeriodic+1; ++ii){
				int itag1 = boundaryPairPeriodicTag[ii].first;//itag1与itag2互为周期边
					...
					if (itag1 == (*iterBoundary).boundaryTag){
						...
						for (iterBoundaryMatch = boundary_->begin(); iterBoundaryMatch != boundary_->end(); ++iterBoundaryMatch){
							if (itag2 == (*iterBoundaryMatch).boundaryTag){
								...
								//这里是减去偏移量，因为偏移量是按照向量方式给的。在使用时则是加上偏移量。需要注意！！
								real pOffsideValue = pOffside.length();
								if (pOffsideValue < parameter_->EPS){
									flag = 0;
									//此处需用flag判断，而不是用指标。因为原来的函数是按指标循环的，
									//如果没有找到，指标会超出范围。这里是按照迭代器循环的，没找到的话指标不会超出范围。
									break;
								}	
							}
						}
						//iterBoundaryMatch -= 1;//20200317 注意自加顺序！！

						//if (jj > boundaryPair[itag2].second){//20200225注意这里的指标！！
						if (flag == 1){//20200225注意这里的指标！！
							std::cout << "	error: fail to match periodic boundary!" << std::endl;
							exit(1);
						}
						(*iterBoundary).matchIndex = (*iterBoundaryMatch).faceIndex;//index为局部指标
		...
		for (iterFace = face_->begin(); iterFace != face_->end(); ++iterFace){
			...
			if ((*iterFace).faceCellIndex[2] < 0){
				for (int ii = 1; ii < parameter_->boundaryNumber+1; ++ii){
					int jj = 1;
					for (iterBoundary = boundary_->begin(); iterBoundary != boundary_->end(); ++iterBoundary){
						if ((*iterBoundary).boundaryTag == ii){ //注意边界标签号！！20200220//20200225 boundaryTag从1开始
							bool isTrue1 = ((*iterBoundary).faceNode[1].first == n1
								&& (*iterBoundary).faceNode[2].first == n2);
							bool isTrue2 = ((*iterBoundary).faceNode[1].first == n2
								&& (*iterBoundary).faceNode[2].first == n1);
							if (isTrue1 || isTrue2){
								(*iterFace).faceCellIndex[2] = (*iterBoundary).boundaryType_;//注意这里！！20200226 -(*iterBoundary).boundaryType_
								//std::cout << (*iterFace).faceCellIndex[2] << std::endl;
								break;//需测试确保break可以跳出for循环！！20200221
							}
							++jj;
						}
					}
	...
	bool grid::saveGrid(const std::string & filename){
		...
		fileOut << "ZONE N =" << parameter_->nodeNumber << "," << "E=" << parameter_->cellNumber << "," << "F=FEPOINT,ET=QUADRILATERAL" << std::endl;
	...
	bool grid::setCellTopology(){
		...
		for (iterFace = face_->begin(); iterFace != face_->end(); ++iterFace){
			...
			(*iterCell).cellFaceNumber += 1;//注意自加位置！！20200225
		...
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			for (int jj = 1; jj < (*iterCell).cellFaceNumber + 1; ++jj){
				int f1 = (*iterCell).cellFaceIndex[jj];//编号
				for (int kk = 1; kk < 3; ++kk){
					iterFace = face_->begin() + f1 - 1;//向量容器位置
					int c1 = (*iterFace).faceCellIndex[kk];
					if (c1 != (*iterCell).index && c1 > 0){
						(*iterCell).cellCellNumber += 1;//注意此处自加位置！！20200225
		...
		//std::cout << "---------------------------------------" << std::endl;
		std::cout << "	inner cell number:" << "\t" << parameter_->innerCellNumber << std::endl;
		std::cout << "	boundary cell number:" << "\t" << parameter_->boundaryCellNumber << std::endl;

		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			(*iterCell).cellCellIndex[0] = (*iterCell).index;//注意20200225！！
	...
	//			(*iterCell).boundaryCellType_ = InnerCell;//注意这里对重构部分的影响！！20200318
	...
	//	}
		...
		std::cout << "------------------------------------------------------------------" << std::endl;
		std::cout << "	wall cell number:" << "\t" << parameter_->boundaryWallCellNumber << std::endl;
		std::cout << "	far field cell number:" << "\t" << parameter_->boundaryFarfieldCellNumber << std::endl;
		std::cout << "	symmetric cell number:" << "\t" << parameter_->boundarySymmetricCellNumber << std::endl;
		std::cout << "	outflow cell number:" << "\t" << parameter_->boundaryOutflowCellNumber << std::endl;
		std::cout << "	inflow cell number:" << "\t" << parameter_->boundaryInflowCellNumber << std::endl;
		std::cout << "	mixed cell number:" << "\t" << parameter_->boundaryMixedCellNumber << std::endl;

		if ((parameter_->boundaryWallCellNumber
			+ parameter_->boundaryFarfieldCellNumber
			+ parameter_->boundarySymmetricCellNumber
			+ parameter_->boundaryOutflowCellNumber
			+ parameter_->boundaryInflowCellNumber
			+ parameter_->boundaryMixedCellNumber)
			!= parameter_->boundaryCellNumber){
			std::cout << "	warning: boundary cell count mistake!" << std::endl;
			//当一个单元有两个边界边时，会出现不匹配情况，这里仅给出参考
			//exit(1);
		}
	...
	bool grid::exportTestGeoInfo(const std::string & filename){
		...
		fileOut << "ZONE N =" << parameter_->nodeNumber << "," << "E=" << parameter_->cellNumber << "," 
			<< "VARLOCATION=([1-2]=NODAL,[3-5]=CELLCENTERED)" << std::endl;
		fileOut << ", " << "DATAPACKING=BLOCK, ZONETYPE=FEQUADRILATERAL" << std::endl;
		//fileOut << "ZONE N =" << parameter_->nodeNumber << "," << "E=" << parameter_->cellNumber << "," << "F=FEPOINT,ET=TRIANGLE" << std::endl;;
		fileOut << std::setprecision(15);
		...
		for (iterNode = node_->begin(); iterNode != node_->end(); ++iterNode){
			fileOut << (*iterNode).nodePhysical.x << "\t";
			if (((*iterNode).index % lineControl) == 0)  fileOut << std::endl;
		}
		fileOut << std::endl;
		for (iterNode = node_->begin(); iterNode != node_->end(); ++iterNode){
		...
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
			...
			if (((*iterCell).index % lineControl) == 0)  fileOut << std::endl;
		}
		fileOut << std::endl;
		for (iterCell = cell_->begin(); iterCell != cell_->end(); ++iterCell){
