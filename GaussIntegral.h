...
namespace ScalarCfv
{
		...
		faceGaussDataVector *faceGaussData)
	{
		...
		dxdKesi = -(physicalPoint[0].second.x - origin.x) * dBaseValue1dKesi + (physicalPoint[1].second.x - origin.x) * dBaseValue2dKesi;
		dydKesi = -(physicalPoint[0].second.y - origin.y) * dBaseValue1dKesi + (physicalPoint[1].second.y - origin.y) * dBaseValue2dKesi;
		...
		real &result)
	{
		...
		for (unsigned int ii = 0; ii < static_cast<int>((*iterGaussData).PG[OG]); ++ii)
		{
			...
			result += getInnerProduct(f[ii], (*iterGaussData).gaussPairVector_[ii].normalVector) * (*iterGaussData).parametricValue[ii].second * (*iterGaussData).parametricArea;
		}
		return true;
	}
		...
		faceGaussDataVector *faceGaussData)
	{
		...
		point p = baseValue1 * physicalPoint[0].second + baseValue2 * physicalPoint[1].second + baseValue3 * physicalPoint[2].second;
		return p;
	}
		...
		faceGaussDataVector *faceGaussData)
	{
		...
		normalVector.x = -(physicalPoint[0].second.y * dBaseValue1dKesi + physicalPoint[1].second.y * dBaseValue2dKesi + physicalPoint[2].second.y * dBaseValue3dKesi);
		normalVector.y = physicalPoint[0].second.x * dBaseValue1dKesi + physicalPoint[1].second.x * dBaseValue2dKesi + physicalPoint[2].second.x * dBaseValue3dKesi;
		...
		faceGaussDataVector *faceGaussData)
	{
		...
		dxdKesi = (physicalPoint[0].second.x - origin.x) * dBaseValue1dKesi + (physicalPoint[1].second.x - origin.x) * dBaseValue2dKesi + (physicalPoint[2].second.x - origin.x) * dBaseValue3dKesi;
		dydKesi = (physicalPoint[0].second.y - origin.y) * dBaseValue1dKesi + (physicalPoint[1].second.y - origin.y) * dBaseValue2dKesi + (physicalPoint[2].second.y - origin.y) * dBaseValue3dKesi;
		...
		cellGaussDataVector *cellGaussData)
	{
		...
		if ((*iterGaussData).cellType_ == Triangle)
		{
			...
			real baseValue2 = kesi;
			real baseValue3 = eta;
			// real baseValue1 = 1 - kesi - eta / std::sqrt(3.);
			// real baseValue2 = kesi - eta / std::sqrt(3.);
			// real baseValue3 = 1 - eta / std::sqrt(3.) * 2;
			p = baseValue1 * physicalPoint[0].second + baseValue2 * physicalPoint[1].second + baseValue3 * physicalPoint[2].second;
		}
		else if ((*iterGaussData).cellType_ == Quadrilateral)
		{
			...
			p = baseValue1 * physicalPoint[0].second + baseValue2 * physicalPoint[1].second + baseValue3 * physicalPoint[2].second + baseValue4 * physicalPoint[3].second;
		}
		return p;
	}
		...
		cellGaussDataVector *cellGaussData)
	{
		...
		if ((*iterGaussData).cellType_ == Triangle)
		{
			...
			// real baseValue2 = kesi - eta/std::sqrt(3.);
			// real baseValue3 = 1-eta/std::sqrt(3.) *2;
			// real dBaseValue1dKesi = -1.0, dBaseValue1dEta = -1.0 / std::sqrt(3.);
			// real dBaseValue2dKesi = 1.0, dBaseValue2dEta = -1.0 / std::sqrt(3.);
			// real dBaseValue3dKesi = 0.0, dBaseValue3dEta = -1.0 / std::sqrt(3.) * 2.;
			...
			J11 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dKesi + (physicalPoint[1].second.x - origin.x) * dBaseValue2dKesi + (physicalPoint[2].second.x - origin.x) * dBaseValue3dKesi;

			J12 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dEta + (physicalPoint[1].second.x - origin.x) * dBaseValue2dEta + (physicalPoint[2].second.x - origin.x) * dBaseValue3dEta;

			J21 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dKesi + (physicalPoint[1].second.y - origin.y) * dBaseValue2dKesi + (physicalPoint[2].second.y - origin.y) * dBaseValue3dKesi;

			J22 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dEta + (physicalPoint[1].second.y - origin.y) * dBaseValue2dEta + (physicalPoint[2].second.y - origin.y) * dBaseValue3dEta;
		}
		else if ((*iterGaussData).cellType_ == Quadrilateral)
		{
			real kesi = (*iterGaussData).parametricValueQuad[idx].first.x;
			...
			J11 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dKesi + (physicalPoint[1].second.x - origin.x) * dBaseValue2dKesi + (physicalPoint[2].second.x - origin.x) * dBaseValue3dKesi + (physicalPoint[3].second.x - origin.x) * dBaseValue4dKesi;
			J12 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dEta + (physicalPoint[1].second.x - origin.x) * dBaseValue2dEta + (physicalPoint[2].second.x - origin.x) * dBaseValue3dEta + (physicalPoint[3].second.x - origin.x) * dBaseValue4dEta;
			J21 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dKesi + (physicalPoint[1].second.y - origin.y) * dBaseValue2dKesi + (physicalPoint[2].second.y - origin.y) * dBaseValue3dKesi + (physicalPoint[3].second.y - origin.y) * dBaseValue4dKesi;
			J22 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dEta + (physicalPoint[1].second.y - origin.y) * dBaseValue2dEta + (physicalPoint[2].second.y - origin.y) * dBaseValue3dEta + (physicalPoint[3].second.y - origin.y) * dBaseValue4dEta;
		}
		...
		cellGaussDataVector *cellGaussData)
	{
		...
		for (iterGaussData = cellGaussData->begin(); iterGaussData != cellGaussData->end(); ++iterGaussData)
		{
			...
			if ((*iterGaussData).cellType_ == Triangle)
			{
				...
				// (*iterGaussData).parametricVolume = 
			}
			else if ((*iterGaussData).cellType_ == Quadrilateral)
			{
				P = 4;
		...
		cellGaussDataVector *cellGaussData)
	{
		...
		if ((*iterGaussData).cellType_ == Triangle)
		{
			...
			p = baseValue1 * physicalPoint[0].second + baseValue2 * physicalPoint[1].second + baseValue3 * physicalPoint[2].second + baseValue4 * physicalPoint[3].second + baseValue5 * physicalPoint[4].second + baseValue6 * physicalPoint[5].second;
		}
		else if ((*iterGaussData).cellType_ == Quadrilateral)
		{
			real kesi = (*iterGaussData).parametricValueQuad[idx].first.x;
			...
			p = baseValue1 * physicalPoint[0].second + baseValue2 * physicalPoint[1].second + baseValue3 * physicalPoint[2].second + baseValue4 * physicalPoint[3].second + baseValue5 * physicalPoint[4].second + baseValue6 * physicalPoint[5].second + baseValue7 * physicalPoint[6].second + baseValue8 * physicalPoint[7].second + baseValue9 * physicalPoint[8].second;
		}
		return p;
	}
		...
		cellGaussDataVector *cellGaussData)
	{
		...
		if ((*iterGaussData).cellType_ == Triangle)
		{
			...
			J11 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dKesi + (physicalPoint[1].second.x - origin.x) * dBaseValue2dKesi + (physicalPoint[2].second.x - origin.x) * dBaseValue3dKesi + (physicalPoint[3].second.x - origin.x) * dBaseValue4dKesi + (physicalPoint[4].second.x - origin.x) * dBaseValue5dKesi + (physicalPoint[5].second.x - origin.x) * dBaseValue6dKesi;

			J12 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dEta + (physicalPoint[1].second.x - origin.x) * dBaseValue2dEta + (physicalPoint[2].second.x - origin.x) * dBaseValue3dEta + (physicalPoint[3].second.x - origin.x) * dBaseValue4dEta + (physicalPoint[4].second.x - origin.x) * dBaseValue5dEta + (physicalPoint[5].second.x - origin.x) * dBaseValue6dEta;

			J21 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dKesi + (physicalPoint[1].second.y - origin.y) * dBaseValue2dKesi + (physicalPoint[2].second.y - origin.y) * dBaseValue3dKesi + (physicalPoint[3].second.y - origin.y) * dBaseValue4dKesi + (physicalPoint[4].second.y - origin.y) * dBaseValue5dKesi + (physicalPoint[5].second.y - origin.y) * dBaseValue6dKesi;

			J22 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dEta + (physicalPoint[1].second.y - origin.y) * dBaseValue2dEta + (physicalPoint[2].second.y - origin.y) * dBaseValue3dEta + (physicalPoint[3].second.y - origin.y) * dBaseValue4dEta + (physicalPoint[4].second.y - origin.y) * dBaseValue5dEta + (physicalPoint[5].second.y - origin.y) * dBaseValue6dEta;
		}
		else if ((*iterGaussData).cellType_ == Quadrilateral)
		{
			real kesi = (*iterGaussData).parametricValueQuad[idx].first.x;
			...
			J11 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dKesi + (physicalPoint[1].second.x - origin.x) * dBaseValue2dKesi + (physicalPoint[2].second.x - origin.x) * dBaseValue3dKesi + (physicalPoint[3].second.x - origin.x) * dBaseValue4dKesi + (physicalPoint[4].second.x - origin.x) * dBaseValue5dKesi + (physicalPoint[5].second.x - origin.x) * dBaseValue6dKesi + (physicalPoint[6].second.x - origin.x) * dBaseValue7dKesi + (physicalPoint[7].second.x - origin.x) * dBaseValue8dKesi + (physicalPoint[8].second.x - origin.x) * dBaseValue9dKesi;

			J12 = (physicalPoint[0].second.x - origin.x) * dBaseValue1dEta + (physicalPoint[1].second.x - origin.x) * dBaseValue2dEta + (physicalPoint[2].second.x - origin.x) * dBaseValue3dEta + (physicalPoint[3].second.x - origin.x) * dBaseValue4dEta + (physicalPoint[4].second.x - origin.x) * dBaseValue5dEta + (physicalPoint[5].second.x - origin.x) * dBaseValue6dEta + (physicalPoint[6].second.x - origin.x) * dBaseValue7dEta + (physicalPoint[7].second.x - origin.x) * dBaseValue8dEta + (physicalPoint[8].second.x - origin.x) * dBaseValue9dEta;

			J21 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dKesi + (physicalPoint[1].second.y - origin.y) * dBaseValue2dKesi + (physicalPoint[2].second.y - origin.y) * dBaseValue3dKesi + (physicalPoint[3].second.y - origin.y) * dBaseValue4dKesi + (physicalPoint[4].second.y - origin.y) * dBaseValue5dKesi + (physicalPoint[5].second.y - origin.y) * dBaseValue6dKesi + (physicalPoint[6].second.y - origin.y) * dBaseValue7dKesi + (physicalPoint[7].second.y - origin.y) * dBaseValue8dKesi + (physicalPoint[8].second.y - origin.y) * dBaseValue9dKesi;

			J22 = (physicalPoint[0].second.y - origin.y) * dBaseValue1dEta + (physicalPoint[1].second.y - origin.y) * dBaseValue2dEta + (physicalPoint[2].second.y - origin.y) * dBaseValue3dEta + (physicalPoint[3].second.y - origin.y) * dBaseValue4dEta + (physicalPoint[4].second.y - origin.y) * dBaseValue5dEta + (physicalPoint[5].second.y - origin.y) * dBaseValue6dEta + (physicalPoint[6].second.y - origin.y) * dBaseValue7dEta + (physicalPoint[7].second.y - origin.y) * dBaseValue8dEta + (physicalPoint[8].second.y - origin.y) * dBaseValue9dEta;
		}
		return std::fabs(J11 * J22 - J12 * J21); //ע��ȷ��Jacobi��ʽ�Ƿ���ȷ����20200218
	}
