...
//		std::cout << "computing time step..." << std::endl;
		...
		for (iterFaceFieldData = faceFieldData->begin(); iterFaceFieldData != faceFieldData->end(); ++iterFaceFieldData){
			...
			if (cr > 0){
				...
				CfvMath::getDiffBaseValue(
					pMid + (*iterFaceFieldData).sideOff,//周期边界条件
					baryCenterJ,
					scaleJ,
					momentJ,
					matrixDiffBaseJ
	...
	bool evolution<O>::excuteWBAPLimiter4th(
		...
		for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
			...
			for (int ii = 1; ii < (*iterCellFieldData).cellCellNumber; ++ii){
				...
				real deltaX = ((*iterCellFieldData).baryCenter.x - ((*iterCellFieldData_).baryCenter.x + (*iterCellFieldData).cellFaceSideOff[ii].x)) / (*iterCellFieldData_).lengthReference.x;
				real deltaY = ((*iterCellFieldData).baryCenter.y - ((*iterCellFieldData_).baryCenter.y + (*iterCellFieldData).cellFaceSideOff[ii].y)) / (*iterCellFieldData_).lengthReference.y;				
				dPhi1[ii] = ((*iterCellFieldData_).scalarVariableTn[1]
					+ 2.0 * (*iterCellFieldData_).scalarVariableTn[3] * deltaX
					+ (*iterCellFieldData_).scalarVariableTn[4] * deltaY
					+ 3.0 * (*iterCellFieldData_).scalarVariableTn[6] * std::pow(deltaX, 2)
					+ 2.0 * (*iterCellFieldData_).scalarVariableTn[7] * deltaX * deltaY
					+ (*iterCellFieldData_).scalarVariableTn[8] * std::pow(deltaY, 2)) * rx;
					
				dPhi2[ii] = ((*iterCellFieldData_).scalarVariableTn[2]
					+ 2.0 * (*iterCellFieldData_).scalarVariableTn[5] * deltaY
					+ (*iterCellFieldData_).scalarVariableTn[4] * deltaX
					+ (*iterCellFieldData_).scalarVariableTn[7] * std::pow(deltaX, 2)
					+ 2.0 * (*iterCellFieldData_).scalarVariableTn[8] * deltaX * deltaY
					+ 3.0 * (*iterCellFieldData_).scalarVariableTn[9] * std::pow(deltaY, 2)) * ry;
			}
	...
	bool evolution<O>::excuteWBAPLimiter4thCR(
		...
		for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
			...
			for (int ii = 1; ii < (*iterCellFieldData).cellCellNumber; ++ii){
				...
				real deltaX = ((*iterCellFieldData).baryCenter.x - ((*iterCellFieldData_).baryCenter.x + (*iterCellFieldData).cellFaceSideOff[ii].x)) / (*iterCellFieldData_).lengthReference.x;
				real deltaY = ((*iterCellFieldData).baryCenter.y - ((*iterCellFieldData_).baryCenter.y + (*iterCellFieldData).cellFaceSideOff[ii].y)) / (*iterCellFieldData_).lengthReference.y;
				dPhi1[ii] = ((*iterCellFieldData_).scalarVariableTnCR[1]
					+ 2.0 * (*iterCellFieldData_).scalarVariableTnCR[3] * deltaX
					+ (*iterCellFieldData_).scalarVariableTnCR[4] * deltaY
					+ 3.0 * (*iterCellFieldData_).scalarVariableTnCR[6] * std::pow(deltaX, 2)
					+ 2.0 * (*iterCellFieldData_).scalarVariableTnCR[7] * deltaX * deltaY
					+ (*iterCellFieldData_).scalarVariableTnCR[8] * std::pow(deltaY, 2)) * rx;

				dPhi2[ii] = ((*iterCellFieldData_).scalarVariableTnCR[2]
					+ 2.0 * (*iterCellFieldData_).scalarVariableTnCR[5] * deltaY
					+ (*iterCellFieldData_).scalarVariableTnCR[4] * deltaX
					+ (*iterCellFieldData_).scalarVariableTnCR[7] * std::pow(deltaX, 2)
					+ 2.0 * (*iterCellFieldData_).scalarVariableTnCR[8] * deltaX * deltaY
					+ 3.0 * (*iterCellFieldData_).scalarVariableTnCR[9] * std::pow(deltaY, 2)) * ry;
			}
...
//------------------------------------------------------------------------------------------
	...
	bool evolution<O>::excuteInnerLoopExplicitSSPRKStep3(
		...
		for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
			(*iterCellFieldData).scalarVariableTn[0] = 
				(1.0 - cof3[iStep]) * (*iterCellFieldData).scalarVariableTm[0]
				+ cof3[iStep] * ((*iterCellFieldData).scalarVariableTn[0] + (*iterCellFieldData).dTPhysical * (*iterCellFieldData).timeMarchingRHSTn);
		}
	...
	bool evolution<O>::excuteInnerLoopExplicitSSPRKStep4(
		...
		if (iStep < 4){	
			for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
				(*iterCellFieldData).scalarVariableTn[0] =
					(1.0 - cof4[iStep]) * (*iterCellFieldData).scalarVariableTm[0]
					+ cof4[iStep] * ((*iterCellFieldData).scalarVariableTn[0] + (*iterCellFieldData).dTPhysical * (*iterCellFieldData).timeMarchingRHSTn);
			}
			for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
				(*iterCellFieldData).timeMarchingRHSRK[iStep] = (*iterCellFieldData).timeMarchingRHSTn;
			}
			...
			for (iterCellFieldData = cellFieldData->begin(); iterCellFieldData != cellFieldData->end(); ++iterCellFieldData){
				(*iterCellFieldData).scalarVariableTn[0] =
					(*iterCellFieldData).scalarVariableTm[0]
					+ cof4[iStep] * (*iterCellFieldData).dTPhysical * (
					(*iterCellFieldData).timeMarchingRHSTn				//current result,	<-> (4)
	...
	bool evolution<O>::getArtificialViscosityTerm(
		...
		for (iterFaceFieldData = faceFieldData->begin(); iterFaceFieldData != faceFieldData->end(); ++iterFaceFieldData){
			...
			for (int gg = 0; gg < static_cast<int>((*iterFaceFieldData).fPG); ++gg){
				...
				//单位化要注意
				uNV[gg] = (1.0 / ((*iterFaceFieldData).gaussPairVector_[gg].normalVector.length()))
					* (*iterFaceFieldData).gaussPairVector_[gg].normalVector;
				...
				if (cr > 0){
					...
					CfvMath::getDiffBaseValue(
						p + (*iterFaceFieldData).sideOff,//周期边界条件
						baryCenterJ,
						scaleJ,
						momentJ,
						matrixDiffBaseJ
